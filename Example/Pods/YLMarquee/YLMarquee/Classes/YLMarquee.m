...
- (void)setText:(NSString *)text{
    if (text) {
        _text = text;
        CGSize textSize = [_text boundingRectWithSize:CGSizeMake(MAXFLOAT, HEIGHT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:self.textFont} context:nil].size;
        
        if (textSize.width + 10 > WIDTH) {
...
- (CABasicAnimation *)scrollAnimation{
    if(!_scrollAnimation){
        ...
        _scrollAnimation.removedOnCompletion = NO;
...
- (CAAnimationGroup *)scrollAnimationGroup{
    if(!_scrollAnimationGroup){
        ...
        _scrollAnimationGroup.removedOnCompletion = NO;
